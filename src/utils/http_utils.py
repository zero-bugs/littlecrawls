...
class HttpClient:
    ...
    def http_retry_executor(self, url, headers: dict, payload=None, method='get', useCookie=False):
        if payload is None:
            payload = {}
        if headers is None:
            headers = {}
        ...
        for num in range(0, 5):
            ...
            try:
                ...
                if method == 'post':
                    ...
                    resp = requests.post(url, data=payload, verify=True, timeout=120, proxies=proxy, headers=headers,
                                         cookies=cookieJar, allow_redirects=False)
                else:
                    resp = requests.get(url, verify=True, timeout=120, proxies=proxy, headers=headers,
                                        cookies=cookieJar,  allow_redirects=False)
                if resp is not None and resp.status_code < 400:
                    self.updateCookie(resp)
                elif resp is not None:
                    print('request exception, status code:{}, method:{}, url:{}'.format(resp.status_code, method, url))
                    print('request cookie:{}'.format(cookieJar))
                    ...
                    print('resp is null, cookie:{}, url:{}'.format(self.currentCookie, url))
                return resp
    ...
    def loginIn(self):
        resp = self.http_retry_executor("{}/login".format(CommonConstant.wall_haven_url), {})
        ...
        resp = self.http_retry_executor("{}/auth/login".format(CommonConstant.wall_haven_url),
                                        {
            ...
            }, method='post', useCookie=True, )
        print('login request code:{}, headers:{}'.format(resp.status_code, resp.headers))
        if resp.status_code == 302:
            print('get redirect location->{} success'.format(resp.headers['location']))
            if resp.headers['location'] == '{}/user/{}'.format(CommonConstant.wall_haven_url, CommonConstant.username):
    ...
    def updateCookie(self, resp):
        ...
        # 每30分钟插入一次cookie
        if (self.lastUpdateCookieTime is None or
            datetime.datetime.now() - self.lastUpdateCookieTime > datetime.timedelta(
                    minutes=30)) and self.currentCookie:
            if self.cookieNamePrefixList[3] in dict_from_cookiejar(self.currentCookie):
                sqliteManager.insert_common_tb('cookies', str(dict_from_cookiejar(self.currentCookie)))
            self.lastUpdateCookieTime = datetime.datetime.now()

    def updateWipikId(self, mp: RequestsCookieJar):
        if mp is None:
            return
        if self.lastUpdateUuidTime is None or datetime.datetime.now() - self.lastUpdateUuidTime > datetime.timedelta(
                minutes=60):
            uid = str(uuid.uuid5(uuid.NAMESPACE_OID,
                                 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Safari/537.36')).replace(
                '-', '')[0:16]
            curtime = round(time.time())

            mp.set(self.traceCookie[0], "{}.{}.{}.{}".format(uid, curtime, curtime, curtime), domain='wallhaven.cc',
                   path='/')
